...
void SpriteAnimation::Update(double dt)
{
	if (m_anim->animActive) // If animation active
	{
		...
		// Find currentFrame
		m_currentFrame = Math::Min(m_anim->endFrame, m_anim->startFrame + static_cast<int>(m_currentTime / frameTime)); // Last frame, Start Fram + Every frame that has past
		...
		if (m_currentTime >= m_anim->animTime)
		{
			...
			{
				// Set Anim to false if no repeat
